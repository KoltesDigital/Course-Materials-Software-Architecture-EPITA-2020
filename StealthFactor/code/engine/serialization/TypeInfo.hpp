...
namespace engine
{
	namespace serialization
	{
		...
		struct TypeInfo
		{
			...
			TypeInfo(Type type, void *methods = nullptr)
				: type{ type }
				, methods{ methods }
			{
			}
		...
		extern template struct TypeInfoValue<unsigned char>;
		extern template struct TypeInfoValue<int>;
		extern template struct TypeInfoValue<float>;
		extern template struct TypeInfoValue<std::string>;
